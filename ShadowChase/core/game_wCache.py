...
class GameMethodCache:
    ...
    def _create_game_state_hash(self, game_state: GameState) -> str:
        ...
        MrX_tickets_serializable = {ticket_type.value: count for ticket_type, count in game_state.MrX_tickets.items()}
        
        # Convert transport types in moves log to serializable format
        MrX_moves_log_serializable = [(pos, transport.value) for pos, transport in game_state.MrX_moves_log]
        ...
        state_dict = {
            ...
            'MrX_visible': game_state.MrX_visible,
            'detective_tickets': detective_tickets_serializable,
            'MrX_tickets': MrX_tickets_serializable,
            'MrX_moves_log': MrX_moves_log_serializable
        }
...
class ShadowChaseGame(Game):
    """Shadow Chase variant of the game with persistent high-performance caching"""
    ...
    def get_valid_moves(self, player: Player, position: int = None, pending_moves: List[Tuple[int, TransportType]] = None) -> Set[Tuple[int, TransportType]]:
        ...
        persistent_cache_key = self.persistent_cache.create_game_cache_key(
            ...
            MrX_visible=self.game_state.MrX_visible,
            detective_tickets=[(k, tuple(v.items())) for k, v in self.game_state.detective_tickets.items()],
            MrX_tickets=tuple(self.game_state.MrX_tickets.items()),
        ...
        if player == Player.DETECTIVES:
            ...
            if position is None:
                position = self.game_state.MrX_position
            result = self._get_valid_MrX_moves(position)
