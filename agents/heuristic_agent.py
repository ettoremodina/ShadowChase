...
class HeuristicDetectiveAgent(DetectiveAgent):
    """Single detective agent that tries to minimize distance to Mr. X's last known position"""
    ...
    def choose_move(self, game: ScotlandYardGame) -> Optional[Tuple[int, TransportType]]:
        """Make a move to get closer to Mr. X's last known position"""
        if self.heuristics is None:
            self.heuristics = GameHeuristics(game)
        else:
            self.heuristics.update_game_state(game)
        ...
        valid_moves = list(self.get_valid_moves(game, current_pos))
